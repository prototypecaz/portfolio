...
function SectionPaint(props) {
  ...
  window.addEventListener('scroll', () => {
    ...
    const containerTop = containerSvg.getBoundingClientRect().top + window.scrollY;
    const containerBottom = containerSvg.getBoundingClientRect().bottom + window.scrollY;
    ...
    paths.forEach(path => {
      ...
      if (scrollPos > containerTop - offset && scrollPos < containerBottom) {
        const effectiveContainerHeight = containerHeight - window.innerHeight + offset;
        const scrollPercent = (scrollPos - containerTop + offset) / effectiveContainerHeight;
...
console.log(scrollPos,containerBottom-800)
    });
  });


    return (
        <div id="containerSvg" style={{height: '1000px',position: 'relative',backgroundColor: 'black',textAlign:'center'}}>
                <svg
      ...
      xmlSpace="preserve"
    >
      <g id="circuit-left" transform="translate(92.128906, 524.812500)" style={{width:'500px'}}>
        <path
          className="st0"
